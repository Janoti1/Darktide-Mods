local mod = get_mod("Murmur Hashdt")

--[[
  
  Library used from
  https://stackoverflow.com/questions/3104722/does-lua-make-use-of-64-bit-integers/7333595#7333595
...
-- conversion of values to be split up so they can be calculated with
...
function i64_toString(a)
  ...
  return string.upper(s3);
end

-- swaps endians and adds converts to binary
-- custom function, credits to Minding
function i64_to_slot_name(a)

  local b = ""

  b = b .. string.char(bit.band(bit.rshift(a.l, 0), 0xff))
  b = b .. string.char(bit.band(bit.rshift(a.l, 8), 0xff))
  b = b .. string.char(bit.band(bit.rshift(a.l, 16), 0xff))
  b = b .. string.char(bit.band(bit.rshift(a.l, 24), 0xff))

  b = b .. string.char(bit.band(bit.rshift(a.h, 0), 0xff))
  b = b .. string.char(bit.band(bit.rshift(a.h, 8), 0xff))
  b = b .. string.char(bit.band(bit.rshift(a.h, 16), 0xff))
  b = b .. string.char(bit.band(bit.rshift(a.h, 24), 0xff))
...
-- multiplication
-- custom functions based on:
-- https://stackoverflow.com/questions/1815367/catch-and-compute-overflow-during-multiplication-of-two-large-integers
-- Credits to Minding
function U32_hi(x)
  return bit.rshift(x, 16)
end
function U32_lo(x)
...
function U32_mul(a, b)
  ...
  x = s2 + a_hi * b_hi + U32_hi(x)

  return x, (bit.lshift(s1, 16) + s0)

end

function i64_mul(a, b)

  local x_hi, x_lo = U32_mul(a.l, b.l)
  local s0 = x_lo

  local t = i64_ax(U32_mul(a.h, b.l))
  local x = i64_add(t, i64(x_hi))
  local s1 = x.l
  local s2 = x.h

  t = i64_ax(U32_mul(a.l, b.h))
  x = i64_add(t, i64(s1))
  s1 = x.l

  -- t = i64_ax(U32_mul(a.h, b.h))
  -- x = i64_add(t, i64_add(i64(s2), x.h))
